...
> [!NOTE]
> 本文主要描述的是“人工调度不同 CLI 进行交叉评审”的通用流程。当前仓库里的实现是基于 `tmux` 的互通与自动投递基础设施，仍处在研究阶段，和纯人工 copy-paste 的评审流在操作细节上会有差异。这里先把流程骨架和约束写清楚，后续会继续围绕 tmux 自动化链路深入探讨与细化。
>
> 也就是说，正文里的步骤和约束是“流程模型”，不是对当前 `tmux` 自动化实现的最终定稿；实现细节后续会在研究阶段继续迭代。
...
如果 reviewer 已经结束推理，但没有主动把最终结论发回 Claude，不要把“沉默”当成流程完成。必须二选一：

1. **结论版重发**
   直接再问 reviewer 一次，只要求输出结论，不要重复分析过程。输出必须包含：
   - `verdict: no issues | findings | mixed`
   - 每条 finding 的 `repeated / new / no issue` 状态
   - 若是重复项，必须说明它为什么仍成立，以及是否有新增证据
   - 若是无问题，必须明确写 `no issue`
2. **主动拉取**
   如果 reviewer 已经回到 `promptReady`，Claude 侧不要等待它“自己发出来”；应立即主动读取当前 pane 的最后输出，并按相同的 `repeated / new / no issue` 规则整理。

这一步的目标不是“多问一句”，而是把 reviewer 的终结信息变成可恢复、可复核、可被下轮引用的结构化结论。没有这一步，后续就容易把半截分析误当最终审查意见。

> [!NOTE]
> 这里的“结论回收”同样先按研究阶段的 tmux 自动化链路来理解：它解决的是当前基础设施里 reviewer 结束后结论容易滞留 pane 的问题，不代表已经把不同 CLI 之间的交叉评审流程完全产品化。
