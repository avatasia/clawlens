# ClawLens Documentation

欢迎来到 ClawLens 插件文档中心。本目录采用分层治理结构，确保文档的权威性与可追溯性。

## 目录索引

### 1. [核心架构与使用](architecture.md)
包含插件的设计哲学、运行时架构、数据流以及详细的使用说明。
- [Architecture Authority](architecture.md)
- [Usage Guide](clawlens-usage.md)

### 2. [开发与部署工作流](CLAWLENS_PLUGIN_DEV_WORKFLOW.md)
开发者必备指南，涵盖本地环境配置、远端部署 SOP 以及兼容性门禁说明。
- [Plugin Dev Workflow](CLAWLENS_PLUGIN_DEV_WORKFLOW.md)
- [QA Lab Test SOP](CLAWLENS_QA_LAB_TEST_SOP.md)
- [Remote Deployment SOP](CLAWLENS_REMOTE_DEPLOYMENT.md)

### 3. [工程治理与质量保证](GOVERNANCE_DOCS_PLAN.md)
定义了文档治理规则、代码索引规范以及自动化校验门禁。
- [Docs Governance Plan](GOVERNANCE_DOCS_PLAN.md)
- [Governance Automation](DOCS_GOVERNANCE_AUTOMATION.md)
- [Transcript Binding Rollback Playbook](CLAWLENS_TRANSCRIPT_BINDING_ROLLBACK_PLAYBOOK.md)

### 4. [方法论与提示词库](PLAYBOOK_ANALYSIS_PROMPT.md)
用于提升 AI Agent 协作效率的专用提示词体系与工程教训总结。
- [Analysis Prompt Playbook](PLAYBOOK_ANALYSIS_PROMPT.md)
- [Model and Effort Selection Playbook](PLAYBOOK_MODEL_AND_EFFORT_SELECTION.md)
- [Engineering Lessons Playbook](PLAYBOOK_ENGINEERING_LESSONS.md)
- [Implementation Guidance](PROMPT_IMPLEMENTATION.md)

## 子目录职责

| 目录 | 职责 |
|---|---|
| [plans/](plans/) | 存放当前仍在讨论、待决或待实施的方案文档。 |
| [research/](research/) | 存放具有参考价值的研究型、排障型文档。 |
| [prompts/](prompts/) | 存放当前活跃的提示词分支文档。 |
| [archive/](archive/) | 历史归档库，包含已完成主题的 History 索引。 |

---
*Last Global Validation: 2026-04-11*
