...
- 若改端口，后续所有 `18789` 相关命令与 URL 必须同步替换为新端口（涉及步骤：3, 5, 7b, 9, 10b, 10c, 11b, 11c）。
...
注意：本步骤会以前台方式占用当前终端。步骤 4 及后续操作需要在**另一个终端窗口**中执行。步骤 5 同样会占用一个终端，因此整个 SOP 至少需要 3 个终端窗口（gateway、QA UI、操作命令）。
...
Docker lane 使用 `qa up` 启动完整 Docker 容器化环境，适用于需要隔离环境的完整回归。

定位：**可选扩展路径，非默认主流程。** 默认 lane 为上文步骤 1–13 的 `qa ui` + 复用 gateway 流程（分析文档 R1 建议，经 T1 验证确认此划分）。Docker lane 尚不具备作为主流程替代的成熟度，应仅在明确需要容器化隔离时使用。

验证背景详见：[RESEARCH_CLAWLENS_QA_LAB_PHASE2_T1_LOCAL_DOCKER_VALIDATION_2026-04-13.md](research/RESEARCH_CLAWLENS_QA_LAB_PHASE2_T1_LOCAL_DOCKER_VALIDATION_2026-04-13.md)
...
- 过程版本（草案）：[IMPLEMENTATION_CLAWLENS_QA_LAB_TEST_SOP_DRAFT_2026-04-13.md](archive/analysis/IMPLEMENTATION_CLAWLENS_QA_LAB_TEST_SOP_DRAFT_2026-04-13.md)
- Phase 2 验证任务单：[IMPLEMENTATION_CLAWLENS_QA_LAB_PHASE2_VALIDATION_TASKS_2026-04-13.md](archive/analysis/IMPLEMENTATION_CLAWLENS_QA_LAB_PHASE2_VALIDATION_TASKS_2026-04-13.md)
